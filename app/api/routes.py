...
from app.schemas.datamodels import TestPlanIn, TestPlanOut
...
async def upload_api_doc(plan: TestPlanIn,db: AsyncSession = Depends(get_session)):
...
async def get_results(plan_id: int,db: AsyncSession = Depends(get_session)):
